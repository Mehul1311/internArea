...
export default function JobListingPage() {
  ...
  const fetchJobs = async (pageNum: number, reset: boolean = false) => {
    try {
      setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="bg-gradient-to-r from-purple-700 via-purple-600 to-indigo-700 py-16 relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
        <div className="max-w-7xl mx-auto px-4 relative z-10 text-center">
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4 tracking-tight">
            Discover Premium <span className="text-purple-200">Jobs</span>
          </h1>
          <p className="text-purple-100 text-lg max-w-2xl mx-auto font-medium">
            Take the next big step in your career. Apply to full-time roles at top companies.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="hidden md:block w-72 bg-white rounded-2xl border border-gray-200 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] sticky top-24">
            <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
              ...
              <button onClick={() => setFilter({ category: "", location: "" })} className="text-xs text-purple-600 font-bold hover:text-purple-800 transition">
                Clear all
              </button>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-3 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition-all"
          ...
          <div className="flex-1 w-full">
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm mb-6 flex items-center justify-between">
              <p className="font-bold text-gray-800">
                {total ? `${total} Jobs Available` : 'Jobs Available'}
              </p>
            </div>
            ...
            <div className="space-y-5">
              {loading && jobData.length === 0 ? (
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-16 text-center flex flex-col items-center">
                  <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center mb-5 text-purple-500">
                    <Filter size={40} />
                  </div>
                  <h3 className="text-2xl font-extrabold text-gray-900 mb-2">No Jobs Found</h3>
                  <p className="text-base text-gray-500 mb-8 max-w-sm">We couldn't find any jobs matching your current filters. Try adjusting your search criteria.</p>
                  <button onClick={() => setFilter({ category: "", location: "" })} className="px-8 py-3 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 hover:shadow-lg hover:-translate-y-0.5 transition-all">
                    Clear Filters
                  </button>
                </div>
              ) : (
                jobData.map((job: any) => (
                  ...
                  >
                    <div className="flex items-center space-x-2 text-purple-700 bg-purple-50/80 px-2.5 py-1 rounded-lg text-[11px] font-bold w-fit mb-5 uppercase tracking-wider">
                    ...
                    <div className="flex justify-between items-start mb-5">
                      <div>
                        <h2 className="text-xl font-extrabold text-gray-900 mb-1.5 group-hover:text-purple-600 transition-colors">
                          {job.title}
                        </h2>
                    ...
                    <div className="flex flex-wrap gap-y-4 gap-x-10 mb-6 text-sm">
                      <div>
                        ...
                        <div className="font-bold text-gray-800">{job.CTC}</div>
                      </div>
                      <div>
                        <div className="flex items-center space-x-1.5 text-gray-400 mb-1.5 text-xs font-semibold uppercase tracking-wide">
                          <Calendar size={14} />
                        ...
                        <div className="font-bold text-gray-800">{job.Experience || '1+ Years'}</div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-gray-100 pt-5 flex justify-end">
                      <Link
                        ...
                        className="text-white bg-purple-600 hover:bg-purple-700 font-bold text-sm px-6 py-2.5 rounded-xl transition-all shadow-sm hover:shadow-md flex items-center gap-1.5"
                      >
              ...
              {hasMore && jobData.length > 0 && !loading && (
                <div className="flex justify-center mt-10">
                  <button 
                    onClick={handleLoadMore} 
                    className="px-8 py-3 bg-white border-2 border-purple-100 text-purple-700 font-bold rounded-xl hover:bg-purple-50 hover:border-purple-200 transition-all shadow-sm hover:shadow-md"
                  >
                    Load More Jobs
                  </button>
                </div>
