...
const index = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        ...
        const userId = user?.id || (storedUser ? JSON.parse(storedUser).id : 1);
  ...
  const filteredapplications = data.filter((application: any) => {
    const companyName = application.company_name || application.company || '';
    const category = application.title || application.category || '';
    const searchmatch =
